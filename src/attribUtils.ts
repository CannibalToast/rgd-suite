...
export function findAttribBase(filePath: string): string | null {
  ...
  for (let d = 0; d < 15; d++) {
    ...
    if (fs.existsSync(dataAttrib)) {
      rememberAttribBase(dirKey, dataAttrib);
      return dataAttrib;
    }
    if (fs.existsSync(attrib)) {
      rememberAttribBase(dirKey, attrib);
      return attrib;
    }
...
function rememberAttribBase(key: string, value: string | null): void {
  if (_attribBaseCache.size >= _ATTRIB_BASE_CACHE_MAX) {
    const firstKey = _attribBaseCache.keys().next().value;
    if (firstKey !== undefined) {
      _attribBaseCache.delete(firstKey);
    }
  }
...
export function makeRgdParentLoader(
  attribBase: string | null,
  dict: HashDictionary,
): RgdParentLoader {
...
export function countEntries(entries: { type: number; value?: unknown }[]): {
  totalEntries: number;
  tableCount: number;
} {
  let totalEntries = 0;
  let tableCount = 0;
  const walk = (arr: typeof entries) => {
    for (const e of arr) {
      totalEntries++;
      const val = e.value as { entries?: typeof entries } | undefined;
      if ((e.type === 100 || e.type === 101) && val?.entries) {
        tableCount++;
        walk(val.entries);
      }
    }
