...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "rgdEditor.convertToText",
      commands.convertToText.bind(commands),
    ),
    vscode.commands.registerCommand(
      "rgdEditor.convertToBinary",
    ...
    vscode.commands.registerCommand(
      "rgdEditor.extractFromSga",
      commands.extractFromSga.bind(commands),
    ),
    vscode.commands.registerCommand(
      "rgdEditor.dumpToLua",
      commands.dumpToLua.bind(commands),
    ),
    vscode.commands.registerCommand(
      "rgdEditor.compileToRgd",
      commands.compileToRgd.bind(commands),
    ),
    vscode.commands.registerCommand(
      "rgdEditor.batchToLua",
      commands.batchToLua.bind(commands),
    ),
    vscode.commands.registerCommand(
      "rgdEditor.batchToRgd",
      commands.batchToRgd.bind(commands),
    ),
    vscode.commands.registerCommand(
      "rgdEditor.selectLocaleFolder",
      async () => {
      ...
      async (filePath: string, line: number = 0, isIcon: boolean = false) => {
        try {
            ...
            if (!targetUri) {
              ...
              if (files.length > 0) {
                ...
                if (searchPath.startsWith("data/")) {
                  ...
                  const subFiles = await vscode.workspace.findFiles(
                    `**/${subPath}`,
                    "**/node_modules/**",
                    1,
                  );
          ...
          vscode.window.showErrorMessage(
            `Failed to open linked file: ${getErrorMessage(e)}`,
          );
        }
      },
    ),
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(async (doc) => {
      ...
      if (
        config.get("autoConvertOnSave", true) &&
        doc.fileName.endsWith(".rgd.txt")
      ) {
