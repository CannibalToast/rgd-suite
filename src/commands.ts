...
export class RgdCommands {
  ...
  async convertToBinary(uri?: vscode.Uri): Promise<void> {
    ...
    if (!uri) {
      const files = await vscode.window.showOpenDialog({
        canSelectMany: false,
        filters: { "RGD Text Files": ["txt"] },
      });
      if (files && files.length > 0) uri = files[0];
    }
    if (!uri) {
      vscode.window.showErrorMessage("No text file selected");
      return;
    }
  ...
  async showInfo(uri?: vscode.Uri): Promise<void> {
    ...
    try {
      ...
      const dict = this.getDictionary();
      const rgdFile = parseRgd(buffer, dict);
      const { totalEntries, tableCount } = countEntries(
        rgdFile.gameData.entries,
      );
      ...
      if (rgdFile.gameData.reference)
        info.push(`Reference: ${rgdFile.gameData.reference}`);
      vscode.window.showInformationMessage(info.join(" | "));
    } catch (error) {
      vscode.window.showErrorMessage(
        `Failed to read info: ${getErrorMessage(error)}`,
      );
    }
  }
  ...
  async extractFromSga(uri?: vscode.Uri): Promise<void> {
    if (!uri) {
      const files = await vscode.window.showOpenDialog({
        canSelectMany: false,
        filters: { "SGA Archives": ["sga"] },
      });
      if (files && files.length > 0) uri = files[0];
    }
    if (!uri) {
      vscode.window.showErrorMessage("No SGA archive selected");
      return;
    }
    ...
    try {
          ...
          if (convertChoice?.startsWith("Yes")) {
            ...
            for (const rgdPath of extracted) {
              ...
              try {
                ...
                const localeMap =
                  LocaleManager.getInstance().getLocaleMap(rgdPath);
                const text = rgdToText(
                  rgdFile,
                  path.basename(rgdPath),
                  localeMap,
                );
                await fs.promises.writeFile(rgdPath + ".txt", text, "utf8");
                converted++;
                progress.report({
                  message: `Converted ${converted}/${extracted.length}`,
                });
              } catch (e) {
                console.error(
                  `Failed to convert ${rgdPath}:`,
                  getErrorMessage(e),
                );
              }
            }
            vscode.window.showInformationMessage(
              `Extracted and converted ${converted} RGD files`,
            );
          } else {
            vscode.window.showInformationMessage(
              `Extracted ${extracted.length} RGD files`,
            );
          }
        },
      );
      ...
      vscode.window.showErrorMessage(
        `Failed to extract: ${getErrorMessage(error)}`,
      );
    }
  }
  ...
  async autoConvertOnSave(document: vscode.TextDocument): Promise<void> {
    ...
    try {
      ...
      vscode.window.setStatusBarMessage(
        `RGD saved: ${path.basename(outputPath)}`,
        3000,
      );
    } catch (error) {
      vscode.window.showWarningMessage(
        `Auto-convert failed: ${getErrorMessage(error)}`,
      );
    }
  }
  ...
  async dumpToLua(uri?: vscode.Uri): Promise<void> {
    ...
    try {
      ...
      await vscode.window.showTextDocument(doc);
      vscode.window.showInformationMessage(
        `Dumped to ${path.basename(outputPath)}`,
      );
    } catch (error) {
      vscode.window.showErrorMessage(
        `Failed to dump to Lua: ${getErrorMessage(error)}`,
      );
    }
  }
  ...
  async compileToRgd(uri?: vscode.Uri): Promise<void> {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to compile to RGD: ${getErrorMessage(error)}`,
      );
    }
  }
  ...
  private async runBatchConvert(opts: {
    ...
    if (files.length === 0) {
      vscode.window.showWarningMessage(
        `No ${opts.inputExt} files found in folder`,
      );
      return;
    }
    ...
    if (workerCount > 0) {
      pool = new BatchConvertWorkerPool(
        workerCount,
        workerScript,
        distPath,
        allDictPaths,
      ...
      async (progress, token) => {
        ...
        const bumpProgress = () => {
          ...
          if (done % reportEvery === 0 || done === total) {
            progress.report({
              message: `${done}/${total}`,
              increment: (100 * reportEvery) / total,
            });
          }
        };
        ...
        try {
          if (pool) {
            ...
            const fallbackParentLoader =
              opts.op === "toLua"
                ? makeLuaParentLoader(attribBase, dict)
                : null;
            const rgdParentLoader =
              opts.op === "toRgd"
                ...
                if (token.isCancellationRequested) {
                  ...
                  try {
                    if (opts.op === "toLua" && fallbackParentLoader) {
                      ...
                      const luaCode = await rgdToLuaDifferential(
                        rgd,
                        fallbackParentLoader,
                      );
                    ...
                    console.error(
                      `Failed to process ${inputFile}: ${getErrorMessage(fallbackErr)}`,
                    );
                    errors++;
                  }
                }
            ...
            for (let i = 0; i < files.length; i++) {
              ...
              try {
                if (opts.op === "toLua") {
                  if (!parentLoader) {
                    throw new Error("parentLoader unavailable");
                  }
                  const rgd = parseRgd(
                    await fs.promises.readFile(inputFile),
                    dict,
                ...
                console.error(
                  `Failed to process ${inputFile}: ${getErrorMessage(e)}`,
                );
                errors++;
              }
              bumpProgress();
            ...
            await Promise.all(
              created.map(async (f) => {
                try {
                  ...
                  console.error(`Failed to delete ${f}:`, getErrorMessage(e));
                }
              }),
            );
            vscode.window.showInformationMessage(
              `Deleted ${deleted} generated ${opts.promptLabel}`,
            );
          }
        }
      },
